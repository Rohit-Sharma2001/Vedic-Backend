...
import { Talks } from '../../schema/schema';
...
export class TalksController {
  constructor(private readonly talksServices: TalksService) { }

  @Post('add')
  async create(
    @Request() req: Request,
    @UploadedFiles() files: { image?: Express.Multer.File[], video_file?: Express.Multer.File[] },
    ...
    try {
      ...
      const result: Partial<Talks> = JSON.parse(decodedData);
      ...
      const createdResult = await this.talksServices.create1(result);

      // // Add host URL to file paths
      // const hostUrl = `http://localhost:3008`;
      // const resultWithUrls = {
      //   ...createdResult.toObject(),
      //   imageUrl: createdResult.image ? `${hostUrl}/${createdResult.image.replace(/\\/g, '/')}` : null,
      //   videoFileUrl: createdResult.video_file ? `${hostUrl}/${createdResult.video_file.replace(/\\/g, '/')}` : null
      // };
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.talksServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.talksServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<Talks> = JSON.parse(decodedData);
      ...
      return await this.talksServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.talksServices.deleteResult(id);
    } catch (error) {
