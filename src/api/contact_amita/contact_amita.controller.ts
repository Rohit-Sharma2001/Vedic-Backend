...
import { ContactAmita } from '../../schema/schema';
...
@Controller('contact_amita')
@UseInterceptors(Base64Interceptor)
export class ContactAmitaController {
  constructor(private readonly contactAmitaServices: ContactAmitaService) { }
  ...
  async create(
    ...
    try {
      ...
      const result: Partial<ContactAmita> = JSON.parse(decodedData);
      ...
      const createdResult = await this.contactAmitaServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.contactAmitaServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.contactAmitaServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<ContactAmita> = JSON.parse(decodedData);
      ...
      return await this.contactAmitaServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.contactAmitaServices.deleteResult(id);
    } catch (error) {
