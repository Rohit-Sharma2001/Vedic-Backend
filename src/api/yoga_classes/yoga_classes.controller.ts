...
import { YogaClasses } from '../../schema/schema';
...
@Controller('yoga_classes')
@UseInterceptors(Base64Interceptor)
export class YogaClassesController {
  constructor(private readonly yogaClassesServices: YogaClassesService) { }
  ...
  async create(
    ...
    try {
      ...
      const result: Partial<YogaClasses> = JSON.parse(decodedData);
      ...
      const createdResult = await this.yogaClassesServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.yogaClassesServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.yogaClassesServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<YogaClasses> = JSON.parse(decodedData);
      ...
      return await this.yogaClassesServices.updateYogaClasses(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.yogaClassesServices.deleteResult(id);
    } catch (error) {
