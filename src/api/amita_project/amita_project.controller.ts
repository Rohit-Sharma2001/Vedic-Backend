...
import { AmitaProject } from '../../schema/schema';
import { Base64Interceptor } from 'src/middlewares/Base64Interceptor/Base64Interceptor';
import { Types } from 'mongoose';
import { Express } from 'express';

@Controller('amita_project')
@UseInterceptors(Base64Interceptor)
export class AmitaProjectController {
  constructor(private readonly amitaProjectServices: AmitaProjectService) { }

  @Post('add')
  async create(
    @Request() req: Request,
    @UploadedFiles() files: { file?: Express.Multer.File[], image?: Express.Multer.File[], additionalImages?: Express.Multer.File[] },
    ...
    try {
      ...
      const result: Partial<AmitaProject> = JSON.parse(decodedData);
      ...
      if (files?.additionalImages && files?.additionalImages.length > 0) {
        result['additionalImages'] = files.additionalImages.map(file => file.path);
      }
      ...
      const createdResult = await this.amitaProjectServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.amitaProjectServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.amitaProjectServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<AmitaProject> = JSON.parse(decodedData);
      ...
      // Handle additional images if uploaded
      if (files?.additionalImages && files?.additionalImages.length > 0) {
        resultUpdates['additionalImages'] = files.additionalImages.map(file => file.path);
      }
      ...
      return await this.amitaProjectServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.amitaProjectServices.deleteResult(id);
    } catch (error) {
