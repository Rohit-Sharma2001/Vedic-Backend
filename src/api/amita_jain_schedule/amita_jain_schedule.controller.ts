...
import { AmitaJainSchedule } from '../../schema/schema';
...
@Controller('amita_jain_schedule')
@UseInterceptors(Base64Interceptor)
export class AmitaJainScheduleController {
  constructor(private readonly amitaJainScheduleServices: AmitaJainScheduleService) { }
  ...
  async create(
    ...
    try {
      ...
      const result: Partial<AmitaJainSchedule> = JSON.parse(decodedData);
      ...
      const createdResult = await this.amitaJainScheduleServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.amitaJainScheduleServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.amitaJainScheduleServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<AmitaJainSchedule> = JSON.parse(decodedData);
      ...
      return await this.amitaJainScheduleServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.amitaJainScheduleServices.deleteResult(id);
    } catch (error) {
