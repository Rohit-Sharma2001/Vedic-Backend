...
import { ClinicBanner } from '../../schema/schema';
...
@Controller('clinic_banner')
@UseInterceptors(Base64Interceptor)
export class ClinicBannerController {
  constructor(private readonly clinicBannerServices: ClinicBannerService) { }
  ...
  async create(
    ...
    try {
      ...
      const result: Partial<ClinicBanner> = JSON.parse(decodedData);
      ...
      const createdResult = await this.clinicBannerServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.clinicBannerServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.clinicBannerServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<ClinicBanner> = JSON.parse(decodedData);
      ...
      return await this.clinicBannerServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.clinicBannerServices.deleteResult(id);
    } catch (error) {
