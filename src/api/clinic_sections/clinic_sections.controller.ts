...
import { ClinicSections } from '../../schema/schema';
...
@Controller('clinic_sections')
@UseInterceptors(Base64Interceptor)
export class ClinicSectionsController {
  constructor(private readonly clinicSectionsServices: ClinicSectionsService) { }
  ...
  async create(
    ...
    try {
      ...
      const result: Partial<ClinicSections> = JSON.parse(decodedData);
      ...
      const createdResult = await this.clinicSectionsServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.clinicSectionsServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.clinicSectionsServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<ClinicSections> = JSON.parse(decodedData);
      ...
      return await this.clinicSectionsServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.clinicSectionsServices.deleteResult(id);
    } catch (error) {
