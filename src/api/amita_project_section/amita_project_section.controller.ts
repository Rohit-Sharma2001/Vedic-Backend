...
import { AmitaProjectSection } from '../../schema/schema';
import { Base64Interceptor } from 'src/middlewares/Base64Interceptor/Base64Interceptor';
import { Types } from 'mongoose';
import { Express } from 'express';

@Controller('amita_project_section')
@UseInterceptors(Base64Interceptor)
export class AmitaProjectSectionController {
  constructor(private readonly amitaProjectSectionServices: AmitaProjectSectionService) { }
  ...
  async create(
    ...
    try {
      ...
      const result: Partial<AmitaProjectSection> = JSON.parse(decodedData);
      ...
      const createdResult = await this.amitaProjectSectionServices.create1(result);
  ...
  async findAll(@Body('data') data: any) {
    try {
      ...
      return this.amitaProjectSectionServices.findAll(page, pageSize);
  ...
  async findOne(@Body('data') data: any) {
    try {
      ...
      return await this.amitaProjectSectionServices.findOneById(id);
    } catch (error) {
  ...
  async update(
    ...
    try {
      ...
      const decodedData = Buffer.from(data, 'base64').toString('utf-8');
      const resultUpdates: Partial<AmitaProjectSection> = JSON.parse(decodedData);
      ...
      return await this.amitaProjectSectionServices.updateArticle(id, resultUpdates);
    } catch (error) {
  ...
  async deleteProduct(@Body('data') data: any) {
    try {
      ...
      return await this.amitaProjectSectionServices.deleteResult(id);
    } catch (error) {
